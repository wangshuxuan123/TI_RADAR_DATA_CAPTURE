...
def angle_processing(radar_cube, fft_size=64, accumulate=True, static_remove=True, radar_type='6843',
                     horizontal=True,window_type_3d=None):
    if radar_type == '1642':
        """
        radar_cube = [(numFrames), numRange_bins, num_vx, numDoppler_bins] 已经经过1dfft或者2dfft处理
        iwr1642虚拟阵元分布
        [0, 1, 2, 3, 4, 5, 6, 7]
        只有方位角信息
        """
        if len(radar_cube.shape) == 4:
            radar_cube = radar_cube.transpose(0, 1, 3, 2)
        elif len(radar_cube.shape) == 3:
            radar_cube = radar_cube.transpose(0, 2, 1)
        if window_type_3d:
            radar_cube = windowing(radar_cube, window_type_3d, axis=-1)

        fft3d_out = np.fft.fft(radar_cube, n=fft_size, axis=-1)

        fft3d_out_abs = np.abs(fft3d_out)
        if static_remove:
            # 去除静态物体
            fft3d_out_abs[:, :, [0, 1, -2, -1], :] = 0
        if accumulate:
            return np.mean(fft3d_out_abs, axis=2), fft3d_out
        return fft3d_out, fft3d_out_abs
    elif radar_type == '6843':
        horizon_array = [11, 9, 7, 5]
        vertical_array = [3, 2, 11, 10]
        if len(radar_cube.shape) == 3:
            radar_cube = radar_cube.transpose(0, 2, 1)
        elif len(radar_cube.shape) == 4:
            radar_cube == radar_cube.transpose(0, 1, 3, 2)
        # if static_remove:
        #     radar_cube[:,[0, 1, -2 ,-1], :] = 0j
        if horizontal:
            radar_cube = radar_cube[:, :, horizon_array]
        else:
            radar_cube = radar_cube[:, :, vertical_array]
        if window_type_3d:
            radar_cube = utils.windowing(radar_cube, window_type_3d, axis=-1)
        fft3d_out = np.fft.fft(radar_cube, n=fft_size)
        fft3d_out_abs = np.abs(fft3d_out)
        if static_remove:
            fft3d_out_abs[:, [0, 1, -2, -1], :] = 0
            #np.fft.fftshift(fft3d_out_abs, axes=1)
        if accumulate:
            return np.mean(fft3d_out_abs, axis=1), fft3d_out
        return fft3d_out_abs, fft3d_out
